...
function random_d() {
    var num = 1;
    do{
        var index = d.indexOf(0);//空出位数组索引
        var moveArr = d_direct[index];
        var arrLength = moveArr.length;
        var randNum = parseInt(Math.floor(Math.random()*arrLength));
        var moveNum = d_direct[index][randNum];
        document.getElementById("d" + d[moveNum]).style.left = d_posXY[index][0] + "px";
        document.getElementById("d" + d[moveNum]).style.top = d_posXY[index][1] + "px";
        d[index] = d[moveNum];
        d[moveNum] = 0;
        num++;
    }while (num<100);
